...
3. **监听端口**：填 `8000` 即可（默认 80 也行，容器已同时兼容 80 与 `PORT` 两个端口，见 §13 Q4）。
...
### Q4：容器状态「Back-off restarting failed container」反复重启
**现象**：镜像构建成功，但实例一直重启，部署日志出现 `Back-off restarting failed container`。
**根因**：TCB 云托管默认监听端口是 `80`，平台的健康检查/流量路由只打「监听端口」。如果容器实际没在那个端口上监听，探针失败 → 容器被杀 → 重启循环。
**已修复**：容器启动器 `run.py` 现在**主应用监听 `PORT`（TCB 注入，默认 8000），并额外在 80 端口回 200 健康检查**。无论 TCB 探针打 80 还是 8000 都能通过。
**你仍要确认**：
- 控制台「监听端口」填 `8000`（或保持默认 80 均可，二者现在都兼容）；
- 重新部署后到「实时日志」看是否出现 `服务启动中，监听端口=...，初始化数据库` 与 `应用就绪`，即代表启动成功、不再重启。
- 若仍重启，请把「实时日志」最新 20-30 行截图，多半是环境变量里开了真实 LLM/取数/COS 但 `requirements.txt` 没装对应包（缺包只在「请求时」报错，不会让容器重启；但可据此确认是否端口已通）。
